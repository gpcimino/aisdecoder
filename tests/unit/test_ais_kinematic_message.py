...
class TestAISKinematicMessage(unittest.TestCase):
    ...
    def test_out_of_range_cog_should_return_none(self):
    ...
    def test_invalid_cog_should_return_error(self):
        ...
        assert_that(m.course_over_ground()).is_equal_to(360.1)
        assert_that(m.validate_course_over_ground()).is_equal_to(err.BAD_COURSE_OVER_GROUND)

    def test_correct_cog_from_sentence_should_validate(self):
    ...
    def test_invalid_sog_should_return_error(self):
        ...
        assert_that(m.speed_over_ground()).is_equal_to(-1)
    ...
    def test_correct_trueheading_should_validate(self):
        ...
        assert_that(m.true_heading()).is_equal_to(90)
        assert_that(m.validate_true_heading()).is_equal_to(err.OK)

    def test_invalid_trueheading_should_return_error(self):
        m = create_msg_123(true_heading=512.0)
        assert_that(m.true_heading()).is_equal_to(512.0)
        assert_that(m.validate_true_heading()).is_equal_to(err.BAD_TRUE_HEADING)

    def test_trueheading_from_sentence_should_validate(self):
        m = AISMessage123.from_sentence(message_1_1["raw"].split(",")[5], 0, "A")
        assert_that(m.true_heading()).is_close_to(message_1_1['parsed']['true_heading'], 0.1)
        assert_that(m.validate_true_heading()).is_equal_to(err.OK)        

    def test_invalid_trueheading_from_sentence_should_return_none(self):
        m = AISMessage123.from_sentence(message_1_3["raw"].split(",")[5], 0, "A")
        assert_that(m.true_heading()).is_none()
        assert_that(m.validate_true_heading()).is_equal_to(err.OK)        
